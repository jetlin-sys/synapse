...
``runtime_env.py`` owns Synapse's app/agent/channel/module runtime layout.
...
class ExecutionEnvSpec:
    """Resolved Synapse-managed execution environment."""
...
def apply_execution_environment(env: dict[str, str], spec: ExecutionEnvSpec) -> dict[str, str]:
    ...
    merged["SYNAPSE_EXECUTION_ENV_SCOPE"] = spec.scope
    merged["SYNAPSE_EXECUTION_ENV_KEY"] = spec.key
    merged["SYNAPSE_EXECUTION_ENV"] = str(spec.venv_path)
    merged["SYNAPSE_EXECUTION_PYTHON"] = str(spec.python_path)
    merged["SYNAPSE_EXECUTION_DEPS_HASH"] = spec.deps_hash
    # Compatibility: existing prompts/tools look for SYNAPSE_AGENT_PYTHON.
    merged["SYNAPSE_AGENT_PYTHON"] = str(spec.python_path)
    merged["SYNAPSE_AGENT_BIN"] = bin_path
