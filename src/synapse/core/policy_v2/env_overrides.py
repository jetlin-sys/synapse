...
1. ``SYNAPSE_POLICY_FILE`` — alternate POLICIES.yaml path (helm
   ConfigMap mount + sidecar render workflows). Resolved BEFORE loading,
   not via this module.
2. ``SYNAPSE_POLICY_HOT_RELOAD`` — force enable/disable hot-reload
   regardless of POLICIES.yaml (rolling deploy: enable hot-reload
   without rewriting the config file).
3. ``SYNAPSE_AUTO_CONFIRM`` — CI/automation: bypass non-destructive
   ...
   not here).
4. ``SYNAPSE_UNATTENDED_STRATEGY`` — override
   ``unattended.default_strategy`` (e.g. CI sets ``deny_all`` to fail
   loud, prod long-runs use ``ask_owner``).
5. ``SYNAPSE_AUDIT_LOG_PATH`` — relocate ``audit.log_path`` to a
...
def _coerce_bool(raw: str) -> bool:
    ...
    falsy — operators making typos like ``SYNAPSE_HOT_RELOAD=yes-please``
...
_REGISTRY: tuple[OverrideSpec, ...] = (
    ...
    OverrideSpec(
        env_name="SYNAPSE_UNATTENDED_STRATEGY",
    ...
    OverrideSpec(
        env_name="SYNAPSE_AUDIT_LOG_PATH",
