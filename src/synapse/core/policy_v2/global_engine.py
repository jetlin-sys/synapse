...
def _resolve_yaml_path() -> Path | None:
    ...
    1. ``SYNAPSE_POLICY_FILE`` 环境变量（操作员用 helm / docker run -e
    ...
    env_path = _os.environ.get("SYNAPSE_POLICY_FILE", "").strip()
    if env_path:
...
def rebuild_engine_v2(
    *,
    ...
    with _lock:
        ...
        # ENV ``SYNAPSE_AUDIT_LOG_PATH`` would change ``_config`` but
