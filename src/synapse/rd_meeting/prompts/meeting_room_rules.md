...
2. **派单**：**必须先**调用 `submit_meeting_work_plan` 提交结构化计划（每项含 `agent_id` / `task` / `reason`），再 `delegate_to_agent` 或 `delegate_parallel`；可并行的只读 / 调研任务优先 `delegate_parallel`（建议带 `plan_item_id` 关联计划条目）。**无协作智能体时**：**不**提交计划、**不** `delegate_*`，见 **§3.2**。
