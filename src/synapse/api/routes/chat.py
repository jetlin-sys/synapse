...
from synapse.core.confirmation_state import ConfirmationDecision, get_confirmation_store
from synapse.core.engine_bridge import engine_stream, is_dual_loop, to_engine
from synapse.core.security_actions import execute_controlled_action
from synapse.core.trusted_paths import grant_session_trust
            ...
            if session_manager and conversation_id:
                try:
                    ...
                    if session:
                        ...
                        try:
                            from synapse.core.feature_flags import (
                                is_enabled as _ff_enabled,
                            )
                            from synapse.core.risk_intent import (
...
def _extract_source_used(event: dict) -> dict | None:
    """Extract Synapse source provenance from tool results.

    Tool handlers prefix result text with ``[SYNAPSE_SOURCE] {json}`` so the
...
def _extract_mcp_call(event: dict) -> dict | None:
    ...
    ``[SYNAPSE_MCP] {json}`` marker so the chat route can emit a stable
...
async def _stream_chat(
    ...
    try:
        ...
        from synapse.core.policy_v2 import DeferredApprovalRequired
...
def _enrich_org_content_with_attachments(
    ...
    from synapse.channels.gateway import MessageGateway
...
async def chat(request: Request, body: ChatRequest):
    ...
    # before falling back to 409.  Header X-Synapse-DoubleTexting can
