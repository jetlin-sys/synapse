...
(:mod:`synapse.core.conversation_metrics`) and the in-flight turn registry
(:mod:`synapse.api.routes.turn_registry`).  Admin uses these to verify
...
from synapse.core.conversation_metrics import snapshot as conversation_metrics_snapshot
...
async def get_conversation_metrics() -> dict:
    ...
    Counter names (see ``synapse.core.conversation_metrics``):
