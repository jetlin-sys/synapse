...
def _normalize_security_profile(profile: str) -> str:
    ...
    from synapse.core.policy_v2.defaults import FACTORY_DEFAULT_PROFILE
...
def _apply_security_profile_defaults(sec: dict[str, Any], profile: str) -> None:
    ...
    """
    from synapse.core.policy_v2.defaults import PROFILE_BUNDLES, profile_bundle
...
def _runtime_env_key_map() -> dict[str, str]:
    ...
    from synapse.config import _PERSISTABLE_KEYS
...
async def write_env(body: EnvUpdateRequest, request: Request):
    ...
    """
    from synapse.utils.atomic_io import safe_write
...
async def save_endpoint(body: SaveEndpointRequest, request: Request):
    ...
    try:
        from synapse.llm.endpoint_validation import (
...
def _trigger_reload(request: Request) -> dict[str, Any]:
    ...
    from synapse.llm.runtime_config import apply_llm_runtime_config
...
async def read_skill_external_allowlist():
    ...
    """
    from synapse.core.security_actions import list_skill_external_allowlist
...
async def write_skills_config(body: SkillsWriteRequest, request: Request):
    ...
    """
    from synapse.core.security_actions import maybe_refresh_skills, set_skill_external_allowlist
...
async def write_security_config(body: SecurityConfigUpdate, replace: bool = False):
    ...
    try:
        from synapse.core.policy_v2.global_engine import reset_policy_v2_layer
...
async def preview_security_config(body: dict | None = None):
    ...
    from synapse.core.policy_v2 import (
        PolicyContext,
        SessionRole,
        ToolCallEvent,
    )
    from synapse.core.policy_v2.enums import ConfirmationMode
    from synapse.core.policy_v2.global_engine import make_preview_engine

    proposed_security = (body or {}).get("security") if isinstance(body, dict) else None

    try:
        if proposed_security is not None:
            from synapse.core.policy_v2.loader import load_policies_from_dict
...
async def read_security_approval_matrix():
    from synapse.core.policy_v2 import ApprovalClass, ConfirmationMode, SessionRole
    from synapse.core.policy_v2.matrix import lookup as lookup_matrix
...
async def read_security_commands():
    ...
    """
    from synapse.core.policy_v2.defaults import default_blocked_commands
...
async def read_permission_mode():
    ...
    """
    try:
        from synapse.core.policy_v2 import read_permission_mode_label
...
async def read_security_audit():
    ...
    try:
        ...
        from synapse.core.policy_v2.audit_chain import (
...
async def reset_death_switch():
    ...
    try:
        from synapse.core.security_actions import (
...
async def read_self_protection():
    ...
    try:
        from synapse.core.policy_v2 import get_death_switch_tracker
...
async def read_user_allowlist():
    ...
    try:
        from synapse.core.security_actions import list_security_allowlist
...
async def add_allowlist_entry(body: dict):
    ...
    try:
        from synapse.core.security_actions import add_security_allowlist_entry
...
async def delete_allowlist_entry(entry_type: str, index: int):
    ...
    try:
        from synapse.core.security_actions import remove_security_allowlist_entry
