...
``~/.synapse/`` (or ``SYNAPSE_ROOT``).  The Python backend always runs
...
def _state_file_path() -> Path:
    return _synapse_home() / "state.json"


def _workspaces_dir() -> Path:
    return _synapse_home() / "workspaces"
...
def _copy_template_files(ws_dir: Path) -> None:
    ...
    pkg_root = Path(__file__).resolve().parent.parent.parent  # synapse package
...
def _find_repo_root() -> Path | None:
    ...
    for _ in range(10):
        if (p / "identity").is_dir() and (p / "src" / "synapse").is_dir():
            return p
