...
from synapse.memory.telemetry import emit_memory_health_event
...
def _desktop_token_expected() -> str:
    return os.environ.get("SYNAPSE_DESKTOP_SESSION_TOKEN", "")
...
def _verify_desktop_token(request: Request) -> None:
    ...
    if not expected:
        ...
        return
    supplied = request.headers.get("X-Synapse-Desktop-Token", "")
...
def _recover_method() -> str:
    ...
    if os.environ.get("SYNAPSE_SQLITE3_EXE"):
        candidates.append(Path(os.environ["SYNAPSE_SQLITE3_EXE"]))
    if os.environ.get("LOCALAPPDATA"):
        candidates.append(Path(os.environ["LOCALAPPDATA"]) / "Programs" / "Synapse" / "sqlite3.exe")
    if os.environ.get("PROGRAMFILES"):
        candidates.append(Path(os.environ["PROGRAMFILES"]) / "Synapse" / "sqlite3.exe")
...
def _sqlite3_executable() -> str | None:
    if os.environ.get("SYNAPSE_SQLITE3_EXE") and Path(os.environ["SYNAPSE_SQLITE3_EXE"]).exists():
        return os.environ["SYNAPSE_SQLITE3_EXE"]
    for env_name in ("LOCALAPPDATA", "PROGRAMFILES"):
        root = os.environ.get(env_name)
        if not root:
            continue
        candidate = Path(root) / "Synapse" / "sqlite3.exe"
        if candidate.exists():
...
async def repair_status(request: Request):
    ...
    return {
        ...
        "backups": _list_candidates("synapse.db.bak.*"),
        "snapshots": _list_candidates("synapse.db.snapshot.*"),
...
async def run_recover(payload: RepairTokenRequest, request: Request):
    def work():
        db = _db_path()
        stamp = time.strftime("%Y%m%d_%H%M%S")
        recovered = _memory_dir() / f"synapse.recovered.{stamp}.db"
        sqlite3_exe = _sqlite3_executable()
        if sqlite3_exe:
            sql_path = _memory_dir() / f"synapse.recovered.{stamp}.sql"
...
# convention: both ``X-Synapse-Desktop-Token`` (Tauri-injected) AND a
...
async def _quiesce_subsystem(request: Request, subsystem: str) -> None:
    ...
    try:
        if subsystem == "token_tracking":
            from synapse.core.token_tracking import shutdown_token_tracking
...
async def degraded_subsystems(request: Request):
    ...
    """
    from synapse.storage.degraded import registry as _registry
