...
# ``from synapse.orgs.runtime import UserCommandTracker``.
...
class OrgRuntime:
    ...
    def _log_finalize_decision(
        ...
        **extra: Any,
        ...
        events.jsonl. DEBUG level: opt-in by lowering synapse.orgs logger.
    ...
    async def _broadcast_ws(self, event: str, data: dict) -> None:
        try:
            from synapse.orgs.event_router import route_org_event
    ...
    def _is_plugin_tool(self, agent: Any, tool_name: str) -> bool:
        ...
        (see ``src/synapse/plugins/api.py``); historically this helper
