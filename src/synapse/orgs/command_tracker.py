...
Lifted from :mod:`synapse.orgs.runtime` so the file can be unit-tested in
...
re-exported by ``synapse.orgs.runtime`` for backward compatibility, so any
``from synapse.orgs.runtime import UserCommandTracker`` import keeps
working.
