...
class SQLiteUnavailable(Exception):
    ...
    :class:`synapse.memory.exceptions.MemoryStorageUnavailable`.
...
def _is_sync_folder_path(path: Path) -> bool:
    ...
    Override with ``SYNAPSE_ALLOW_SYNC_FOLDER_DB=1`` for power users
    who accept the risk.
    """
    text = str(path).lower()
    if os.environ.get("SYNAPSE_ALLOW_SYNC_FOLDER_DB") == "1":
        return False
