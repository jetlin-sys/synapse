"""Monorepo / bundled-tree fallback loader for ``synapse_plugin_sdk``.

Why this exists
---------------
Plugin entry modules import ``synapse_plugin_sdk`` directly (for
...
plugin fails with::
    ...
    ModuleNotFoundError: No module named 'synapse_plugin_sdk'
...
``synapse-plugin-sdk/src/synapse_plugin_sdk`` source tree and prepends
...
- **No imports of synapse.*** — plugins/manager imports this at boot, so
...
_SDK_PACKAGE_NAME = "synapse_plugin_sdk"
_SDK_SOURCE_DIRNAME = "synapse-plugin-sdk"
_INJECTED_PATH: Path | None = None


def ensure_plugin_sdk_on_path() -> Path | None:
    """Make ``synapse_plugin_sdk`` importable, returning the package path.
    ...
    3. Walk parent directories of this file looking for
       ``<repo>/synapse-plugin-sdk/src/synapse_plugin_sdk/__init__.py``.
    ...
    for parent in here.parents:
        ...
        if candidate_pkg.is_file():
            ...
            logger.info(
                "synapse_plugin_sdk not installed; injected monorepo source from %s",
                candidate_src,
            )
            return _INJECTED_PATH
