...
You cannot judge a diagram from JSON alone. After generating or editing the Excalidraw JSON, you **should** render it to PNG, view the image, and fix what you see — in a loop until it's right — **when no other visual preview is available**.

### When the PNG loop may be skipped

If the delivery target is a **Markdown + Excalidraw embed** (e.g. a React-based preview that loads `.excalidraw` next to the document), treat that viewer as the validation surface: open the diagram there, fix the JSON, refresh — **without** requiring `render_excalidraw.py` / PNG for every iteration. The `.excalidraw` file remains the canonical artifact. Still use the PNG loop for static docs, print, or CI gates where no Excalidraw-capable renderer exists.
