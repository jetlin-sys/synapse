...
// ── Mock data (migrated from TicketDetails.tsx) ───────────────
const SIMILAR_TICKETS: TicketRecord[] = [
  {
    ...
    matchRate: 94, type: 'bug', product: '电商平台',
    modules: ['购物车', '结算'], effort: '3人天', devTickets: 4, complexity: 'high', assignee: '李明',
  },
  {
    key: '2', id: 'ISSUE-1988',
    title: '订单金额统计报表尾差修复',
    ...
    matchRate: 81, type: 'bug', product: '数据报表',
    modules: ['订单', '报表'], effort: '2人天', devTickets: 2, complexity: 'medium', assignee: '张三',
  },
  {
    key: '3', id: 'FEAT-3012',
    ...
    matchRate: 67, type: 'feature', product: '基础平台',
    modules: ['支付', '账单', '风控'], effort: '8人天', devTickets: 7, complexity: 'high', assignee: '王五',
  },
  {
    key: '4', id: 'ISSUE-2102',
    ...
    matchRate: 89, type: 'bug', product: '中间件',
    modules: ['缓存', '分布式锁'], effort: '4人天', devTickets: 3, complexity: 'high', assignee: '赵六',
  },
  {
    key: '5', id: 'FEAT-3045',
    ...
    matchRate: 78, type: 'feature', product: '支付结算',
    modules: ['对账', '性能'], effort: '5人天', devTickets: 5, complexity: 'medium', assignee: '陈七',
  },
];

// ── Helper fns (migrated from TicketDetails.tsx) ─────────────
...
// ── Expanded row description (same as TicketDetails) ─────────
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
        <div
          style={{
            marginTop: 1, flexShrink: 0, width: 18, height: 18, borderRadius: 4,
...
export interface RequirementAnalysisPanelProps {
  ...
  /** 节点类型颜色（tailwind text 色值字符串） */
  nodeTypeColor: string;
  /** 所属阶段名称 */
  stageName: string;
  /** 执行产物 / 交互区 JSX（由父组件 renderNodeOutput 生成） */
  nodeOutput: React.ReactNode;
}
...
export function RequirementAnalysisPanel({
  ...
  const sortedTickets = [...SIMILAR_TICKETS].sort((a, b) => b.matchRate - a.matchRate);

  // ���─ Summary stats for section header ───────────────────────
  ...
  const summaryStats = [
    { icon: <BarChart2 size={12} color="#a78bfa" />, label: '相似工单', value: `${total} 条`, color: '#c4b5fd', bg: 'rgba(139,92,246,0.08)', border: 'rgba(139,92,246,0.2)' },
    { icon: <AlertCircle size={12} color="#f87171" />, label: '需求 / 故障', value: `${featureCount} / ${bugCount}`, color: '#fca5a5', bg: 'rgba(239,68,68,0.07)', border: 'rgba(239,68,68,0.18)' },
    { icon: <Layers size={12} color="#60a5fa" />, label: '最关联模块', value: topModule, color: '#93c5fd', bg: 'rgba(59,130,246,0.07)', border: 'rgba(59,130,246,0.18)' },
    { icon: <Clock size={12} color="#34d399" />, label: '工作量预估', value: '2 人天', color: '#6ee7b7', bg: 'rgba(52,211,153,0.07)', border: 'rgba(52,211,153,0.18)' },
  ];

  // ── Table columns (migrated from TicketDetails.tsx) ─────────
  const columns: TableColumnsType<TicketRecord> = [
    {
      title: '单号', dataIndex: 'id', key: 'id', width: 118,
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
          {mods.map(m => (
            <Tag key={m} style={{
              ...
              border: '1px solid rgba(99,102,241,0.3)', fontSize: 10,
              marginInlineEnd: 0, padding: '0 5px', lineHeight: '18px',
            }}>
              {m}
            </Tag>
          ))}
    ...
    {
      title: '匹配度', dataIndex: 'matchRate', key: 'matchRate', width: 108,
      render: (rate: number) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <div style={{
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            minWidth: 40, height: 18, borderRadius: 10,
            background: getMatchBg(rate), border: `1px solid ${getMatchBorder(rate)}`, padding: '0 6px',
          }}>
            <Text style={{ color: getMatchColor(rate), fontSize: 11, fontFamily: 'ui-monospace, monospace', fontWeight: 600 }}>
              {rate}%
            </Text>
          </div>
          <Progress
    ...
    },
    {
      ...
      render: (count: number) => (
        <div style={{
          display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
          minWidth: 26, height: 18, borderRadius: 10,
          background: 'rgba(56,189,248,0.1)', border: '1px solid rgba(56,189,248,0.28)', padding: '0 7px',
        }}>
          <Text style={{ color: '#7dd3fc', fontSize: 11, fontFamily: 'ui-monospace, monospace', fontWeight: 600 }}>{count}</Text>
        </div>
      ),
    },
    {
      title: '复杂度', dataIndex: 'complexity', key: 'complexity', width: 60,
      render: (v: string) => {
        const cfg = complexityConfig[v] ?? complexityConfig['medium'];
        return (
          <Tag style={{
            background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.border}`,
            fontSize: 10, marginInlineEnd: 0, padding: '0 7px',
          }}>
            {cfg.label}
          </Tag>
        );
        ...
        <div className="p-5">
          ...
          <Tabs
            ...
            items={[
              {
                key: 'output',
                label: <span className="flex items-center gap-1.5 text-xs"><Activity className="w-3 h-3" />任务产出物</span>,
                children: (
