...
interface Ticket {
  ...
  currentStage: number;
  currentNode: string;
  status: 'processing' | 'human_intervention' | 'pending' | 'completed';
  owner: string;
...
const MOCK_TICKETS: Ticket[] = [
  {
    ...
    tokens: 425600,
    runTime: '2h 45m',
    description: '实现核心调度模块，支持多Agent并发执行任务并处理状态回传。目前在沙箱环境中发现并发锁死异常。',
    createdAt: '2026-04-13 09:00:00'
  },
  {
    id: 'TICKET-2026-0413-002',
    branch: 'feature/knowledge-base-sync',
    title: '产品知识库实时同步策略',
    currentStage: 2,
    currentNode: 'solution_review',
    status: 'processing',
    owner: 'Admin',
    urgency: 'medium',
    tokens: 128400,
    runTime: '45m',
    description: '设计并开发基于向量数据库的实时同步机制，保证产品知识库与代码变动的一致性。',
    createdAt: '2026-04-13 10:30:00'
  },
  {
    id: 'TICKET-2026-0413-003',
    branch: 'fix/sandbox-timeout-issue',
    title: '修复沙箱执行超时未释放问题',
    currentStage: 0,
    currentNode: 'pending',
    status: 'pending',
    ...
    tokens: 0,
    runTime: '0m',
    description: '线上偶尔出现沙箱资源无法释放导致资源池耗尽，需要排查超时处理逻辑。',
    createdAt: '2026-04-13 11:15:00'
  },
  {
    id: 'TICKET-2026-0413-004',
    ...
    tokens: 890200,
    runTime: '5h 20m',
    description: '提升单元测试生成的覆盖率，支持复杂分支逻辑的MOCK生成。已完成测试和控熵评审，等待组长合并。',
    createdAt: '2026-04-12 16:00:00'
  },
  {
    id: 'TICKET-2026-0411-001',
    branch: 'feature/mock-data-generator',
    title: 'Mock数据生成器支持自定义规则',
    currentStage: 5,
    currentNode: 'leader_review',
    status: 'completed',
    owner: 'Admin',
    ...
    tokens: 54000,
    runTime: '1h 20m',
    description: '开发人员可以根据需求定义Mock数据生成的规则。',
    createdAt: '2026-04-11 09:30:00'
  }
];
...
const getNodeStateGlobal = (ticket: Ticket | null, nodeId: string): NodeState => {
  ...
  if (ticket.status === 'human_intervention') {
    ...
    if (node.type.includes('human')) return 'human_intervention';
    return 'error';
  }
  return 'pending';
};
...
export const OrderManagement: React.FC = () => {
  ...
  const [activeTicketId, setActiveTicketId] = useState<string>(MOCK_TICKETS[0].id);
  ...
  useEffect(() => {
    ...
    // Small delay to ensure the DOM is fully rendered and any initial layout is settled
    const timeoutId = setTimeout(() => {
      const activeNodeElement = document.getElementById(`node-${activeTicket.currentNode}`);
      const container = containerRef.current;
      
      if (activeNodeElement && container) {
        // Use getBoundingClientRect to calculate the exact pixel distance on the screen
        const nodeRect = activeNodeElement.getBoundingClientRect();
        const containerRect = container.getBoundingClientRect();
        
        // Find the center point of the active node on the screen
        const nodeCenter = nodeRect.left + (nodeRect.width / 2);
        // Find the center point of the scroll container on the screen
        const containerCenter = containerRect.left + (containerRect.width / 2);
        
        // The difference is exactly how much we need to scroll the container
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between bg-slate-900 p-3 rounded-lg border border-slate-800">
                ...
                {state === 'active_human' ? <Badge status="warning" text="审核中" /> : <Badge status="success" text="已通过" />}
              </div>
              <div className="flex items-center justify-between bg-slate-900 p-3 rounded-lg border border-slate-800">
                <div className="flex items-center gap-3">
                  ...
                  <div key={stage.id} className="flex relative z-10 px-6 h-full border-l border-slate-800/50 border-dashed">
                    ...
                    <div className="flex ml-16 h-full items-center">
                      {stage.nodes.map((node, nIdx) => {
                        ...
                        // Generate mock processing stats based on node type
                        const nodeHash = node.id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                        const timeStr = isHuman 
                            ...
                            >
                              <div className="flex items-start justify-between mb-2">
                                ...
                                <div className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-black/40 border border-white/5">
                                  {node.type.includes('human') || node.type === 'ai_exception' ? (
          ...
          <div className="space-y-6 pt-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800">
                ...
                <div className="font-medium text-blue-400">{SOP_STAGES[selectedTicketForModal.currentStage].name}</div>
              </div>
              <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800">
