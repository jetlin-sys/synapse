...
Disable with config ``mock_mode=0`` or env ``SYNAPSE_ECOM_MOCK=0``.
Force-always with ``mock_mode=1`` or ``SYNAPSE_ECOM_MOCK=1``.
"""
...
def mock_mode_from_env() -> str | None:
    v = os.environ.get("SYNAPSE_ECOM_MOCK", "").strip().lower()
...
def mock_delay_seconds() -> float:
    try:
        return float(os.environ.get("SYNAPSE_ECOM_MOCK_DELAY", "2.0"))
    except ValueError:
        return 2.0
