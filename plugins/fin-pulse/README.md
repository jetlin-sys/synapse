...
> Finance news radar for Synapse. Three canonical modes —
...
```bash
cd D:/Synapse/plugins/fin-pulse
# the 5-asset UI bundle ships vendored; no build step required
```

Restart the Synapse host and confirm in Plugin Manager that
...
4. **AI Brain** — fin-pulse reuses the host LLM factory; configure
   provider / model / temperature in Synapse → Settings → Models.
...
- **Ask tab** — 7 agent tool cards with JSON samples and
  "copy natural-language prompt" buttons. Paste into the Synapse
...
- Rows also show up in the main Synapse **Scheduler** panel, but you
...
8. In Synapse main chat, ask *"今天美股有什么大事"* → Brain
...
Same as Synapse — see `D:/Synapse/LICENSE`.
