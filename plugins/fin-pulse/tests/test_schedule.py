...
# refuses to import ``synapse.plugins.api`` (StrEnum). We stub the
...
def _load_plugin_module() -> types.ModuleType:
    ...
    # Stub synapse.plugins.api so ``from synapse.plugins.api import
    # PluginAPI, PluginBase`` succeeds.
    synapse_pkg = types.ModuleType("synapse")
    plugins_pkg = types.ModuleType("synapse.plugins")
    api_mod = types.ModuleType("synapse.plugins.api")
    ...
    synapse_pkg.plugins = plugins_pkg

    sys.modules.setdefault("synapse", synapse_pkg)
    sys.modules.setdefault("synapse.plugins", plugins_pkg)
    sys.modules.setdefault("synapse.plugins.api", api_mod)
