...
from synapse.plugins.api import PluginBase
...
_LOG = logging.getLogger(f"synapse.plugin.{PLUGIN_ID}")
...
class Plugin(PluginBase):
    ...
    def _resolve_dashscope_endpoint(
        ...
        try:
            ...
            _LOG.info(
                "[%s] synapse.relay not importable (%s); keeping per-plugin DashScope endpoint",
                PLUGIN_ID,
                exc,
            )
            return api_key, ""
