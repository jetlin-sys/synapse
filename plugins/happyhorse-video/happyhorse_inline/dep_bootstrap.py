...
1. **Probe** the three places host-managed deps could already live and
   ...
   - ``~/.synapse/modules/happyhorse-video/site-packages/`` — host's
   ...
   - ``<sys.executable>/_internal/Lib/site-packages/`` — bundled
     ...
     occasionally seeded by older Synapse versions).
...
2. **Import-or-install**: if the import still fails, run ``pip install``
   ...
   ``~/.synapse/modules/happyhorse-video/site-packages/`` so the install
...
def _patch_simplejson_jsondecodeerror() -> bool:
    ...
    Some PyInstaller Synapse builds expose a ``simplejson`` module that
...
def _synapse_root() -> Path:
    """Mirror ``runtime_env._get_synapse_root`` without importing it.

    The plugin must NOT depend on ``synapse.*`` internals (those move
    between releases) so we re-derive the same value: ``$SYNAPSE_ROOT``
    if set, else ``~/.synapse``.
    """
...
def _candidate_site_dirs(plugin_dir: Path | None) -> list[Path]:
    ...
    # while the Synapse process still had a .pyd file locked (seen with
    ...
    cands.append(_synapse_root() / "modules" / "happyhorse-video" / "site-packages")
...
def _stale_module_prefixes(import_name: str) -> tuple[str, ...]:
    ...
    if import_name == "oss2":
        return (
            ...
            # Synapse Desktop builds already import a bundled Crypto package
...
def _resolve_python_executable() -> str:
    ...
    ``synapse-server.exe`` which intercepts CLI args and refuses
...
def ensure_importable(
    ...
    *,
    ...
    # environments and fixes Synapse Desktop builds where ``requests``
