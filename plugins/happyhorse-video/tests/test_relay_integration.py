...
Plugins can point at a relay registered in Synapse's shared relay
...
4. When ``synapse.relay`` cannot be imported (degraded environment)
...
# ─── Fake synapse.relay module so tests don't depend on PYTHONPATH ──
...
def _install_stub_relay_module(monkeypatch, *, refs_by_name=None, raises=None):
    """Inject a fake ``synapse.relay`` module into sys.modules.
    ...
    monkeypatch.setitem(sys.modules, "synapse", SimpleNamespace(relay=fake_pkg))
    monkeypatch.setitem(sys.modules, "synapse.relay", fake_pkg)
...
def test_plugin_local_relay_url_overrides_without_host_registry(monkeypatch):
    ...
    must not require synapse.relay or a global LLM relay entry."""
    monkeypatch.setitem(sys.modules, "synapse.relay", None)
    c = HappyhorseDashScopeClient(
        _read_settings_factory(
            api_key="sk-direct",
...
def test_synapse_relay_import_failure_falls_back_gracefully(monkeypatch):
    """When the bundled / installed plugin runs in an environment
    that does not expose synapse.relay, the client must NOT crash —
