...
class Plugin(PluginBase):
    ...
    def _resolve_effective_endpoint(
        ...
        *,
        ...
        Synapse's shared relay registry (see ``synapse.relay``),
        ...
        ship without the synapse host package.
        """
        ...
        try:
            ...
            logger.info(
                "tongyi-image: synapse.relay not importable (%s); "
    ...
    def _register_routes(self, router: APIRouter) -> None:
        ...
        async def optimize_prompt_endpoint(body: PromptOptimizeBody) -> dict:
            ...
            if not brain:
                return {
                    "ok": False,
                    "error": "LLM 不可用：主进程未注入 brain（请确认 Synapse 已正常启动）。",
                }
            try:
