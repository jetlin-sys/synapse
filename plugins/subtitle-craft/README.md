...
Synapse 的 AI 字幕全生命周期插件 — 一个插件涵盖 **自动字幕 → 翻译 → 修复 → 烧制** 的完整链路。
...
把整个 `plugins/subtitle-craft/` 目录放到 Synapse 的 `plugins/` 下,在主界面启用即可。第一次启动会自动创建 SQLite 数据库 `data/plugins/subtitle-craft/subtitle_craft.db`。
