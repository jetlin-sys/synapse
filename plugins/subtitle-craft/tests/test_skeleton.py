...
- Red-line grep guards (matches Gate 0 in ``docs/subtitle-craft-plan.md``):
  ...
  ``plugins-archive`` / ``_shared`` / ``sdk.contrib`` / ``synapse_plugin_sdk.contrib``,
...
def test_plugin_skeleton_import():
    try:
        ...
        pytest.skip("synapse SDK not available in test env")
...
_FORBIDDEN_IMPORT_PATTERNS = [
    ...
    re.compile(r"^\s*from\s+synapse_plugin_sdk\.contrib\b", re.MULTILINE),
