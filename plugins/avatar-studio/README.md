...
avatar-studio 跟随 Synapse 主仓发布，不需要额外 `pip install`。开发态：
...
目标 Synapse 实例的 `plugins/` 目录即可。所有依赖（vendored helpers、UI
Kit）都在插件内部，不依赖 `synapse_plugin_sdk.contrib`、不依赖
`/api/plugins/_sdk/*`。
...
打开 Synapse → 插件 → Avatar Studio → 进入插件 UI → **Settings** Tab：
...
Pixelle A10）。这一点也是 avatar-studio 区别于早期 Synapse 插件最重要的
工程改进 —— 用户不必为了改一个字符配置而重启整个 Synapse 主进程。
...
```bash
# 0. 确保 Synapse 已启动并加载了 avatar-studio
