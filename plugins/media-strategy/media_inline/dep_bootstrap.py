...
bootstrap: probe plugin-local ``deps/`` first, then a private Synapse
...
def _synapse_root() -> Path:
    raw = os.environ.get("SYNAPSE_ROOT", "").strip()
    return Path(raw) if raw else Path.home() / ".synapse"
...
def _candidate_dirs(plugin_dir: Path | None) -> list[Path]:
    ...
    dirs.append(_synapse_root() / "modules" / PLUGIN_ID / "site-packages")
