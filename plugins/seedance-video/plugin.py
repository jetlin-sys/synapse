...
class Plugin(PluginBase):
    ...
    def _resolve_effective_ark_endpoint(
        ...
        *,
        ...
        Import is lazy so the plugin still loads when synapse.relay
        ...
        try:
            ...
            logger.info(
                "seedance-video: synapse.relay not importable (%s); "
        ...
        async def optimize_prompt_endpoint(body: PromptOptimizeBody) -> dict:
            ...
            if not brain:
                return {
                    ...
                    "message": "LLM 不可用：主进程未注入 brain（请确认 Synapse 已正常配置 LLM）。",
                }
            try:
