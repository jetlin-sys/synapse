...
class Plugin(PluginBase):
    """Synapse plugin entry for guided PPT generation."""
...
def _default_settings() -> dict[str, str]:
    return {
        ...
        # official DashScope endpoint. See src/synapse/relay/.
...
def _apply_relay_overrides(settings: dict[str, str]) -> dict[str, str]:
    ...
    Failure modes:
      ...
      * synapse.relay missing       -> noop, settings keep per-plugin
    ...
    if not relay_name:
        return settings
    try:
        from synapse.relay import apply_relay_override
