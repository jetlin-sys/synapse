...
> 前端现状：`apps/setup-center/src/views/rd-manage/MeetingRoomView.tsx` → `MeetingRoomBoard`（当前为 Mock UI，SOP 节点与工单管理页一致）
...
```text
open_meeting(scope_type, scope_id):
...
| GET | `/api/dev/meeting-rooms/{room_id}` | 读 `room_state` + `history` + `dev.status` |
| POST | `/api/dev/meeting-rooms/open` | `{ scope_type, scope_id }` |
| POST | `/api/dev/meeting-rooms/{room_id}/intervene` | 人工指令 / 聊天 |
| GET/PUT | `/api/dev/work/{scope_type}/{scope_id}/dev.status` | 流水线状态 CRUD |
| GET | `/api/dev/work-orders/.../meeting-summary` | 工单侧**只读**聚合（§4.6） |
| GET/PUT | `/api/dev/meeting-room-config` | 组织级会议室配置 |
...
| 项 | 现状 | 目标 |
|----|------|------|
| `MeetingRoomView` | 仅渲染 `MeetingRoomBoard` | 传入 `synapseApiBase` 并拉取真实列表 |
| 数据源 | `INITIAL_ROOMS` Mock | `GET /api/dev/meeting-rooms`（`dev.status` + 只读 `userwork` 标题） |
| SOP 定义 | 与 `OrderManagement` 各一份硬编码 | 共享 `rd-sop/constants` |
| 配置抽屉 | 无 | 编辑 `meeting_room_config`（节点补充说明、Profile/Skill/LLM 引用） |
| 工单联动 | 无 | 工单行操作「进入会议室」带 `scope` / `stage` 路由参数 |
...
| 资源 | 建议路径 |
|------|----------|
| SOP Manifest | `src/synapse/rd_sop/manifest.json` |
...
| 项 | 说明 |
|----|------|
| 默认 SKILL ID | `whalecloud-dev-tool-meeting-room` |
| 文件位置 | `skills/whalecloud-dev-tool-meeting-room/SKILL.md` |
...
| 角色裁剪 | `host` 视角隐藏 §5（Worker 规范），`worker` 视角隐藏 §4（Host 工作循环） |
| 能力卡片 | 自动展开 `worker_profile_ids`：名称 / 角色 / 端点 / 核心技能 / 主张 |

SKILL 内核覆盖用户提出的 8 项要求：
...
| `node_overrides[].node_intent` | `NODE_INTENT` | **会议目标**，不可留空 | Manifest `NODE_INTENTS` |
...
| 模块 | 职责 |
|------|------|
| `skills/whalecloud-dev-tool-meeting-room/SKILL.md` | 会议室协作宪法（含模板变量） |
| `src/synapse/rd_meeting/room_skill.py` | SKILL 加载 / 角色裁剪 / 能力卡片渲染 / 上下文装配 |
| `src/synapse/rd_meeting/config_store.py` | v2 配置读写（部分写、向后兼容） |
| `src/synapse/rd_meeting/binding.py` | `resolve_node_binding` 返回 `host_llm_endpoint_key` / `worker_llm_endpoint_key` / `meeting_skill_id` |
| `src/synapse/rd_meeting/orchestrator.py` | `_prewarm_meeting_room` + `_configure_meeting_agent`：注入端点 + SKILL + cwd |
| `apps/setup-center/.../MeetingRoomConfigDrawer.tsx` | "会议室全局配置" 块：双端点选择 + SKILL 卡片 |
...
| 2026-05-19 | **Phase 2 实现**：`meeting_room_config.json` + `GET/PUT meeting-room-config`；`binding` / `manifest`；`POST .../run-node` 编排（dry-run 或 Agent 池）；配置抽屉与「执行当前节点」入口。 |
